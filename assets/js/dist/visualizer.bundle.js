...
var sanitiseAndParse = async (maybeResponse) => {
  if (!maybeResponse) {
    throw new Error("Empty Gemini response");
  }
  const rawText = typeof maybeResponse.text === "function" ? await maybeResponse.text() : maybeResponse.text;
...
var getProjectedGrowthData = async (topic, providedKey) => {
  ...
  try {
    ...
    const response = await client.models.generateContent({
      ...
      config: schema ? { systemInstruction, responseMimeType: "application/json", responseSchema: schema } : { systemInstruction, responseMimeType: "application/json" }
    });
